...
class Other extends Base
{
    ...
    public function saveEditPass()
    {
        ...
        $up=[
            'password'=>password_hash($ref_password, PASSWORD_DEFAULT),
        ];
        $db = MysqlPool::defer();
        $res=$db->where('id',$this->user['id'])->update('user',$up);
        if ($res){
    ...
    public function saveAdd()
    {
        ...
        if (empty($username) || empty($password)){
            return $this->writeJson(10004,'必要参数不能为空');
        }
        $db = MysqlPool::defer();
        $exist=$db->where('username',$username)->getOne('user');
        if (!empty($exist)){
        ...
        $add=[
            ...
            'status'=>'online',
            'nickname'=>'admin'.mt_rand(0000,9999),
            'sign'=>'空空如也',
        ];
        $res=$db->insert('user',$add);
        if ($res){
            return $this->writeJson(Status::CODE_OK,'添加成功');
        }
        return $this->writeJson(10004,'添加失败');
