...
class User extends Base
{
    ...
    private function addOrder($data)
    {
        ...
        //查询dmr转出费率
        $getData=$db->where('type','DMR')->getOne('config','out_rate');
        $rate=$getData['out_rate'];//dmr转出费率
        $ser_money=bcmul($data['number'],$rate,4);//计算手续费
        ...
        if ($money['d_frozen'] >= $data['number']) {
            ...
            $to_up = [
                'd_frozen' =>bcsub(bcadd($to_money['d_frozen'], $data['number'], 4),$ser_money,4),//全部使用冻结给对方冻结-手续费
            ];//对方账户
        } else {
            //冻结不足
            //计算需要多少可用填充
            ...
            $to_up = [
                ...
                'dmr' => bcsub(bcadd($to_money['dmr'], $need, 4),$ser_money,4),//+可用-手续费
            ];//对方账户
        }
        //开启事务
        $db->startTransaction();
        ...
        $add = [
            ...
            'orderid' => $orderid,
            'datetime' => date('Y-m-d H:i:s'),
            'ser_money'=>$ser_money,//dmr转出手续费
            'ac_money'=>bcsub($data['number'],$ser_money,4),//对方实际到账总
        ];
        ...
        if ($res1 && $res2 && $res3){
            ...
            $to_insert=[
                ...
                'money'=>bcsub($data['number'],$ser_money,4),//变动金额
                'gmoney'=>bcadd($to_allMoney, bcsub($data['number'],$ser_money,4), 4),//+
                'datetime'=>date('Y-m-d H:i:s'),
            ...
            /*****************************记录平台分成*********************************/
            $this->editMoney($ser_money,'DMR',$data['uid']);
    ...
    {
        $db = MysqlPool::defer();
        $list=$db->where('status',1)->get('notic',[0,5],'id,title,content,datetime');
    ...
    public function getContent()
    {
        ...
        $db = MysqlPool::defer();
        $data=$db->where('id',$id)->getOne('notic','id,title,content,datetime');
        return $this->writeJson(Status::CODE_OK,'SUCCESS',$data);
    }
