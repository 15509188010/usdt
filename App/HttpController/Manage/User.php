...
class User extends Base
{
    ...
    {
        $db = MysqlPool::defer();
        $plat_money=$db->where('id',1)->getOne('plat_money','id,money,u_money,d_money');

        $data=[
            'user'=>$this->user['username'],
            'money'=>$plat_money['money'],
            'd_money'=>$plat_money['d_money'],
            'u_money'=>$plat_money['u_money'],
        ];
        return $this->render('User/main',['data'=>$data]);
    }
